        ...
        <div class="bg-white shadow rounded-lg">
            ...
            <div class="px-4 py-5 sm:p-6 border-b border-gray-200">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('attribute-groups.show', $attributeGroup->group_id) }}" class="text-gray-400 hover:text-gray-600">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                        </svg>
                    </a>
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Edit Attribute Group</h1>
            ...
            <!-- Form -->
            <form method="POST" action="{{ route('attribute-groups.update', $attributeGroup->group_id) }}" class="px-4 py-5 sm:p-6">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <!-- Basic Information -->
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($entityTypes as $type)
                                        <option value="{{ $type->entity_type_id }}" {{ old('entity_type_id', $attributeGroup->entity_type_id) == $type->entity_type_id ? 'selected' : '' }}>
                                            {{ $type->type_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    name="group_code"
                                    value="{{ old('group_code', $attributeGroup->group_code) }}"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input
                                    type="text"
                                    name="group_name"
                                    value="{{ old('group_name', $attributeGroup->group_name) }}"
                            ...
                            <div class="flex items-center">
                                <label class="flex items-center">
                                    <input
                                        ...
                                        name="is_active"
                                        value="1"
                                        {{ old('is_active', $attributeGroup->is_active) ? 'checked' : '' }}
                    ...
                    @if($attributeGroup->attributes->count() > 0)
                        <div class="bg-blue-50 rounded-lg p-4">
                            <h3 class="text-sm font-medium text-blue-800 mb-2">Current Attributes ({{ $attributeGroup->attributes->count() }})</h3>
                            ...
                            <p class="text-xs text-blue-600 mt-2">
                                Note: Changing entity type may affect attribute assignments.
                            </p>
                        </div>
                    @endif
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <a href="{{ route('attribute-groups.show', $attributeGroup->group_id) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                        Cancel
                    </a>
