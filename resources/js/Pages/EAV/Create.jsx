...
export default function Create({ entityTypes, attributes, parent, entityTypeId, parentId }) {
    ...
    const { data, setData, post, processing, errors, reset } = useForm({
        ...
        sort_order: 0,
        ...Object.fromEntries(
            attributes.map(attr => [`attr_${attr.attribute_id}`, ''])
        )
    });
    ...
    useEffect(() => {
        if (selectedEntityType) {
            const filteredAttributes = attributes.filter(attr => 
                attr.entity_type_id == selectedEntityType || attr.entity_type_id === null
            );
            setCurrentAttributes(filteredAttributes);
            
            // Reset form data for new entity type
            const newFormData = {
            ...
            filteredAttributes.forEach(attr => {
                newFormData[`attr_${attr.attribute_id}`] = '';
            });
            
            setFormData(newFormData);
        }
    }, [selectedEntityType, attributes, parentId]);
    ...
    return (
        <AppLayout title="Create Entity - EAV">
                    ...
                    <div className="bg-white shadow rounded-lg">
                        ...
                        <form onSubmit={handleSubmit} className="px-4 py-5 sm:p-6">
                            <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select
                                                ...
                                                className={`mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 ${
                                                    errors.entity_type_id ? 'border-red-300' : ''
                                                }`}
                                            >
                                            ...
                                            {errors.entity_type_id && (
                                                <p className="text-xs text-red-600 mt-1">{errors.entity_type_id}</p>
                                            )}
                                        </div>
                                ...
                                {/* Dynamic Attributes */}
                                {selectedEntityType && currentAttributes.length > 0 && (
                                    <div className="bg-blue-50 rounded-lg p-4">
                                        <h2 className="text-lg font-medium text-gray-900 mb-4">
                                            Attributes for {entityTypes.find(t => t.entity_type_id == selectedEntityType)?.type_name}
                                        </h2>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {currentAttributes.map(renderAttributeField)}
                                        </div>
                                    </div>
                                )}
                            ...
                            <div className="mt-6 flex justify-end space-x-3">
                                ...
                                >
                                    {processing ? 'Creating...' : 'Create Entity'}
                                </button>
                            </div>
                        </form>
                    </div>
